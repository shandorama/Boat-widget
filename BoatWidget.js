...
/**
 * BoatWidget loader - Saltholmen -> Köpstadsö.
 *
 * This tiny stub should rarely need to change. Its only job is to download
 * the actual widget logic (BoatWidgetCore.js) from GitHub, save it as a
 * separate local file, and run it immediately - in THIS same run, not the
 * next one. That's what makes a manual ▶️ Play show a just-pushed fix right
 * away, instead of needing two runs (one to download, one to apply).
 *
 ...
 * content (e.g. BoatWidgetCore_a1b2c3.js) rather than a fixed name, and
 * importModule()'d by that exact name. Scriptable's importModule() can
 ...
 * live in BoatWidgetCore.js - this file has nothing worth editing.
 */

const CORE_BASE_NAME = "BoatWidgetCore";
const CORE_URL = "https://raw.githubusercontent.com/shandorama/Boat-widget/main/BoatWidgetCore.js";
